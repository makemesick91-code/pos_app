<?php

namespace App\Http\Resources\Api\V1\Admin;

use App\Models\CommercialLaunchSignoff;
use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

/**
 * @mixin CommercialLaunchSignoff
 *
 * Sprint 20 — presents a commercial launch signoff. No secrets are exposed.
 */
class CommercialLaunchSignoffResource extends JsonResource
{
    /**
     * @return array<string, mixed>
     */
    public function toArray(Request $request): array
    {
        return [
            'id' => $this->id,
            'commercial_launch_run_id' => $this->commercial_launch_run_id,
            'signoff_reference' => $this->signoff_reference,
            'signer_user_id' => $this->signer_user_id,
            'signer_name' => $this->signer_name,
            'signer_role' => $this->signer_role,
            'decision' => $this->decision,
            'notes' => $this->notes,
            'evidence_reference' => $this->evidence_reference,
            'signed_at' => $this->signed_at,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
        ];
    }
}
