...
/**
 * Sprint 21 — public website risk lifecycle.
 *
 * Owns create/update/accept-risk/close for public website risks and summarizes
 ...
 * expiry. Secret-looking values are stripped from free-text/metadata.
 */
class PublicWebsiteRiskGovernanceService
{
     ...
     */
    public function create(array $attributes, ?User $actor = null): PublicWebsiteRisk
    {
        return PublicWebsiteRisk::query()->create([
            'risk_reference' => (string) ($attributes['risk_reference'] ?? $this->generateReference()),
            'area' => $this->normalizeArea((string) ($attributes['area'] ?? PublicWebsiteRisk::AREA_OTHER)),
            'severity' => $this->normalizeSeverity((string) ($attributes['severity'] ?? '')),
            'status' => $this->normalizeStatus((string) ($attributes['status'] ?? PublicWebsiteRisk::STATUS_OPEN)),
     ...
     * @param array<string,mixed> $attributes
     */
    public function update(PublicWebsiteRisk $risk, array $attributes, ?User $actor = null): PublicWebsiteRisk
    {
        $map = [
     ...
     */
    public function acceptRisk(PublicWebsiteRisk $risk, array $data, ?User $actor = null): PublicWebsiteRisk
    {
        ...
        $requires = in_array($risk->severity, (array) config('public_website.accepted_risk_requires_expiry_for', []), true);
        ...
        $risk->status = PublicWebsiteRisk::STATUS_ACCEPTED_RISK;
    ...
    public function close(PublicWebsiteRisk $risk, ?User $actor = null): PublicWebsiteRisk
    {
        $risk->status = PublicWebsiteRisk::STATUS_CLOSED;
        $risk->save();
    ...
    {
        $now ??= Carbon::now();
        $blocking = (array) config('public_website.blocking_risk_severities', []);
        $watch = (array) config('public_website.watch_risk_severities', []);

        $open = PublicWebsiteRisk::query()->open()->get();

        $openBySeverity = [];
        foreach (PublicWebsiteRisk::SEVERITIES as $severity) {
            $openBySeverity[$severity] = $open->where('severity', $severity)->count();
        }

        $openBlockingUnaccepted = $open
            ->filter(fn (PublicWebsiteRisk $r) => in_array($r->severity, $blocking, true))
            ->count();

        $openWatchNoMitigation = $open
            ->filter(fn (PublicWebsiteRisk $r) => in_array($r->severity, $watch, true) && trim((string) $r->mitigation) === '')
            ->count();
        ...
        $expiredAcceptedRiskBlocking = PublicWebsiteRisk::query()
            ->where('status', PublicWebsiteRisk::STATUS_ACCEPTED_RISK)
            ->whereIn('severity', $blocking)
            ->whereNotNull('accepted_risk_expires_at')
            ->where('accepted_risk_expires_at', '<', $now)
            ->count();

        $expiredAcceptedRiskWatch = PublicWebsiteRisk::query()
            ->where('status', PublicWebsiteRisk::STATUS_ACCEPTED_RISK)
    ...
    private function normalizeSeverity(string $severity): string
    {
        ...
        if (! in_array($severity, PublicWebsiteRisk::SEVERITIES, true)) {
    ...
    private function normalizeStatus(string $status): string
    {
        ...
        if (! in_array($status, PublicWebsiteRisk::STATUSES, true)) {
    ...
    private function normalizeArea(string $area): string
    {
        ...
        return in_array($area, PublicWebsiteRisk::AREAS, true) ? $area : PublicWebsiteRisk::AREA_OTHER;
    }

    private function generateReference(): string
    {
        return 'WRISK-'.now()->format('Ymd').'-'.strtoupper(Str::random(6));
    }
}
