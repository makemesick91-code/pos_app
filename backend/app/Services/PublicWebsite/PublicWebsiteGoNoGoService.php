<?php

namespace App\Services\PublicWebsite;

use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Artisan;

/**
 * Sprint 21 — public website GO / WATCH / NO_GO aggregation.
 *
 * Combines the cumulative prior-sprint gate contract (Sprint 13 release, 14
 * RC/UAT, 15 deployment/field, 16 monitoring/hypercare, 17 stabilization, 18
 * closure/handover, 19 production operations, 20 commercial launch commands
 * registered), the public website documentation contract, the Android release
 * readiness script, and the full public website readiness evaluation (pages,
 * landing content, lead governance, SEO metadata, privacy/cookie, risk review,
 * signoff review) into a single public website launch decision.
 *
 *   NO_GO — any required prior gate/command/doc is missing, no approved landing
 *           page version, a missing required page, missing privacy/terms
 *           readiness, an open CRITICAL/HIGH risk without a valid accepted risk,
 *           or a rejected signoff.
 *   WATCH — no blocking failure but a warning exists (open MEDIUM risk with
 *           mitigation, approved-with-risk signoff, or a non-critical SEO /
 *           content / package alignment warning).
 *   GO    — every signal passes.
 *
 * Never prints secrets, never deploys, never bills a real customer, never opens
 * public signup, never sends real alerts, never runs Android Gradle.
 */
class PublicWebsiteGoNoGoService
{
    public const STATUS_PASS = 'PASS';
    public const STATUS_WARN = 'WARN';
    public const STATUS_FAIL = 'FAIL';

    public const DECISION_GO = 'GO';
    public const DECISION_WATCH = 'WATCH';
    public const DECISION_NO_GO = 'NO_GO';

    public function __construct(
        private readonly PublicWebsiteReadinessService $readiness,
    ) {}

    /**
     * @return array<string,mixed>
     */
    public function evaluate(?Carbon $now = null): array
    {
        $readiness = $this->readiness->evaluate($now);

        $signals = [
            $this->commandsSignal(),
            $this->docsSignal(),
            $this->androidScriptSignal(),
            $this->decisionSignal('public_pages', (string) $readiness['public_pages']['decision']),
            $this->decisionSignal('landing_page', (string) $readiness['landing_page']['decision']),
            $this->decisionSignal('lead_governance', (string) $readiness['lead_governance']['decision']),
            $this->decisionSignal('seo_metadata', (string) $readiness['seo_metadata']['decision']),
            $this->decisionSignal('privacy_cookie', (string) $readiness['privacy_cookie']['decision']),
            $this->decisionSignal('risk_review', (string) $readiness['risk_review']['decision']),
            $this->decisionSignal('content_signoff', (string) $readiness['content_signoff']['decision']),
            $this->decisionSignal('public_website_readiness', (string) $readiness['decision']),
        ];

        return [
            'decision' => $this->decision($signals),
            'signals' => $signals,
            'gates' => $this->gateReferences(),
            'public_website_readiness' => $readiness,
        ];
    }

    /**
     * @param array<int,array{status:string}> $signals
     */
    private function decision(array $signals): string
    {
        foreach ($signals as $signal) {
            if ($signal['status'] === self::STATUS_FAIL) {
                return self::DECISION_NO_GO;
            }
        }

        foreach ($signals as $signal) {
            if ($signal['status'] === self::STATUS_WARN) {
                return self::DECISION_WATCH;
            }
        }

        return self::DECISION_GO;
    }

    /** @return array{key:string,status:string,message:string} */
    private function signal(string $key, string $status, string $message): array
    {
        return ['key' => $key, 'status' => $status, 'message' => $message];
    }

    private function decisionSignal(string $key, string $decision): array
    {
        return match ($decision) {
            self::DECISION_NO_GO => $this->signal($key, self::STATUS_FAIL, "{$key} is NO_GO."),
            self::DECISION_WATCH => $this->signal($key, self::STATUS_WARN, "{$key} is WATCH."),
            default => $this->signal($key, self::STATUS_PASS, "{$key} is GO."),
        };
    }

    private function commandsSignal(): array
    {
        $required = (array) config('public_website.required_commands', []);
        $registered = array_keys(Artisan::all());
        $missing = array_values(array_diff($required, $registered));

        return $missing === []
            ? $this->signal('required_commands', self::STATUS_PASS, count($required).' prior-sprint commands registered.')
            : $this->signal('required_commands', self::STATUS_FAIL, 'Missing required commands: '.implode(', ', $missing));
    }

    private function docsSignal(): array
    {
        $required = (array) config('public_website.required_docs', []);
        $missing = [];
        foreach ($required as $doc) {
            if (! is_file($this->repoRoot().'/'.ltrim((string) $doc, '/'))) {
                $missing[] = $doc;
            }
        }

        return $missing === []
            ? $this->signal('public_website_docs', self::STATUS_PASS, count($required).' public website docs present.')
            : $this->signal('public_website_docs', self::STATUS_FAIL, 'Missing public website docs: '.implode(', ', $missing));
    }

    private function androidScriptSignal(): array
    {
        $script = (string) config('public_website.android_release_readiness_script', '');
        $exists = $script !== '' && is_file($this->repoRoot().'/'.ltrim($script, '/'));

        return $exists
            ? $this->signal('android_release_readiness', self::STATUS_PASS, 'Android release readiness script present.')
            : $this->signal('android_release_readiness', self::STATUS_FAIL, 'Missing Android release readiness script: '.$script);
    }

    /**
     * @return array<string,bool>
     */
    private function gateReferences(): array
    {
        $registered = array_keys(Artisan::all());
        $gates = (array) config('public_website.prior_sprint_gates', []);

        $out = [];
        foreach ($gates as $name => $commands) {
            $out[$name] = array_diff((array) $commands, $registered) === [];
        }

        return $out;
    }

    private function repoRoot(): string
    {
        return (string) (realpath(base_path('..')) ?: base_path('..'));
    }
}
