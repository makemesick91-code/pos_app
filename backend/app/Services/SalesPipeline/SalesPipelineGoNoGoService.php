...
/**
 * Sprint 22 — sales pipeline GO / WATCH / NO_GO aggregation.
 *
 ...
 * website commands registered), the sales pipeline documentation contract, the
 * Android release readiness script, and the full sales pipeline readiness
 * evaluation (stages, lead intake, assignment, activity, qualification,
 * onboarding handover, risk review, signoff review) into a single decision.
 *
 *   NO_GO — any required prior gate/command/doc is missing, a missing canonical
 *           stage, an open CRITICAL/HIGH risk without a valid accepted risk, or a
 *           rejected signoff.
 *   WATCH — no blocking failure but a warning exists (open MEDIUM risk, an
 *           approved-with-risk signoff, or a missing signoff role).
 *   GO    — every signal passes.
 *
 * Never prints secrets, never deploys, never bills a real customer, never creates
 * a tenant/user/subscription/device from a lead, never integrates a real CRM,
 * never sends real WhatsApp/email/Slack, never runs Android Gradle.
 */
class SalesPipelineGoNoGoService
{
    ...
    public function __construct(
        private readonly SalesPipelineReadinessService $readiness,
    ) {}
    ...
    public function evaluate(?Carbon $now = null): array
    {
        ...
        $signals = [
            ...
            $this->decisionSignal('canonical_stages', (string) $readiness['canonical_stages']['decision']),
            $this->decisionSignal('lead_intake', (string) $readiness['lead_intake']['decision']),
            $this->decisionSignal('assignment_governance', (string) $readiness['assignment_governance']['decision']),
            $this->decisionSignal('activity_tracking', (string) $readiness['activity_tracking']['decision']),
            $this->decisionSignal('qualification', (string) $readiness['qualification']['decision']),
            $this->decisionSignal('onboarding_handover_readiness', (string) $readiness['onboarding_handover_readiness']['decision']),
            ...
            $this->decisionSignal('sales_pipeline_readiness', (string) $readiness['decision']),
        ];
        ...
        return [
            ...
            'sales_pipeline_readiness' => $readiness,
        ];
    }
    ...
    {
        $required = (array) config('sales_pipeline.required_commands', []);
    ...
    private function docsSignal(): array
    {
        ...
        return $missing === []
            ? $this->signal('sales_pipeline_docs', self::STATUS_PASS, count($required).' sales pipeline docs present.')
            : $this->signal('sales_pipeline_docs', self::STATUS_FAIL, 'Missing sales pipeline docs: '.implode(', ', $missing));
    }

    private function androidScriptSignal(): array
    {
        $script = (string) config('sales_pipeline.android_release_readiness_script', '');
    ...
    {
        $registered = array_keys(Artisan::all());
        $gates = (array) config('sales_pipeline.prior_sprint_gates', []);
