...
/**
 * Sprint 22 — a preserved sales pipeline sign-off record. A REJECTED signoff
...
return new class extends Migration
{
    ...
    {
        Schema::create('sales_pipeline_signoffs', function (Blueprint $table) {
            $table->id();
            $table->string('signoff_reference')->unique();
            $table->foreignId('signer_user_id')->nullable()->constrained('users')->nullOnDelete();
            $table->string('signer_name')->nullable();
            $table->string('signer_role'); // OWNER | SALES | TECHNICAL | OPERATIONS | LEGAL_PRIVACY | ONBOARDING
    ...
    {
        Schema::dropIfExists('sales_pipeline_signoffs');
    }
};
