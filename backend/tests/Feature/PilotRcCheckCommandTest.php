...
/**
 * Sprint 14 — pilot:rc-check command contract.
 */
class PilotRcCheckCommandTest extends TestCase
{
    use RefreshDatabase;

    public function test_command_is_registered(): void
    {
        $this->assertArrayHasKey('pilot:rc-check', Artisan::all());
    }
    ...
    public function test_go_when_ready(): void
    {
        ...
        $exit = Artisan::call('pilot:rc-check', ['--json' => true]);
    ...
    public function test_strict_mode_fails_on_watch(): void
    {
        ...
        $exit = Artisan::call('pilot:rc-check', ['--json' => true, '--strict' => true]);
    ...
    {
        config(['app.key' => 'base64:PILOTSECRETKEYVALUE123456789012345678901234=']);

        Artisan::call('pilot:rc-check', ['--json' => true]);

        $this->assertStringNotContainsString('PILOTSECRETKEYVALUE', Artisan::output());
    }
}
