...
<head>
    ...
    <title>@yield('title', 'Konsol Pemilik') · Aish POS</title>
    <style>{!! file_get_contents(resource_path('css/aish-tokens.css')) !!}</style>
    <style>
        :root { --shadow: 0 12px 32px rgba(11, 16, 32, .08); --sidebar-w: 252px; }
        * { box-sizing: border-box; }
        ...
        .tenant-chip {
            font-size: 12px; color: var(--aish-text-on-dark-muted);
            border: 1px solid rgba(255,255,255,.14); border-radius: 10px; padding: 8px 10px;
        }
        .tenant-chip strong { color: #fff; display: block; font-size: 13px; }
    ...
    <div class="shell" id="shell" data-nav="closed">
        ...
        <aside class="sidebar" id="sidebar" aria-label="Navigasi utama">
            <div class="brand">
                ...
                <span>
                    ...
                    <span class="brand-sub">Konsol Pemilik</span>
                </span>
            </div>
            <div class="tenant-chip">
                Bisnis Anda
                <strong>{{ $context->tenant->name }}</strong>
                <span>{{ $context->tenant->code }}</span>
            </div>
            <nav class="side-nav" aria-label="Menu">
                <a href="{{ route('owner.dashboard') }}" @if(request()->routeIs('owner.dashboard')) aria-current="page" @endif>Dashboard</a>
        ...
        <div class="main">
            <header class="topbar">
                ...
                <strong>@yield('title', 'Konsol Pemilik')</strong>
                <span class="spacer"></span>
                <span class="who">{{ $context->user->email }}</span>
