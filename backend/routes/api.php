...
use App\Http\Controllers\Api\V1\Admin\AdminBillingGovernanceController;
...
Route::prefix('v1')->group(function () {
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        Route::prefix('admin')->middleware('platform.admin')->group(function () {
            ...
            // Sprint 30 — billing invoice generation & payment collection
            // governance. Platform admin only (BIL-R007). Reads are cross-tenant
            // governance visibility; mutations (generate / record payment / mark
            // failed / cancel) are all audit-logged with redacted metadata
            // (BIL-R008). Invoice amounts come from the plan pricing source of
            // truth (BIL-R003); generation is idempotent per tenant + period
            // (BIL-R002); a failed/cancelled payment never marks an invoice paid
            // (BIL-R010). There is deliberately NO tenant/public route that can
            // mutate invoice/payment state.
            // Prefixed `tenant-billing` to keep a clean namespace boundary with the
            // Sprint 23 `billing/*` collection-governance surface (no route collision).
            Route::get('/tenant-billing/invoices', [AdminBillingInvoiceController::class, 'index']);
            Route::get('/tenants/{tenant}/tenant-billing/invoices', [AdminBillingInvoiceController::class, 'forTenant']);
            Route::post('/tenants/{tenant}/tenant-billing/invoices/generate', [AdminBillingInvoiceController::class, 'generate']);

            Route::post('/tenant-billing/invoices/{invoice}/payments', [AdminBillingPaymentController::class, 'store']);
            Route::post('/tenant-billing/payments/{payment}/mark-failed', [AdminBillingPaymentController::class, 'markFailed']);
            Route::post('/tenant-billing/payments/{payment}/cancel', [AdminBillingPaymentController::class, 'cancel']);
